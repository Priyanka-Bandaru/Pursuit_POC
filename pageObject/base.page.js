...
exports.BasePage = class BasePage {
  constructor(page, test) {
    ...
    this.bestSellerItemButton = page.locator(
      "//input[@id='bestselling-best seller']"
    );
    ...
    this.productAddToCartButton = page.locator(
      "(//button[contains(@id,'addToCartButtonOrTextIdFor')])[1]"
    );
    ...
    this.itemAtCart = (itemName) =>
      page.locator("//div[contains(text(),'" + itemName + "')]");
    this.removeItem = (itemName) =>
      page.locator("(//button[contains(@aria-label,'" + itemName + "')])[3]");
    this.goToHome = page.locator("//a[text()='Go to homepage']");
    this.assertPassword= page.locator("(//span[text()='Please enter a valid password'])[2]");
  }
  ...
  async signInIntoApplication(userName, password) {
    await actions.executeStep(
      this.test,
      this.homePageSignBtn,
      "click",
      "click on sign button in landing page"
    );
    await actions.executeStep(
      this.test,
      this.homePageSecondSignIn,
      "click",
      "click on sign button in landing page model"
    );
    await actions.executeStep(
      this.test,
      this.emailTab,
  ...
  async serachForItem(currentItem) {
    await actions.executeStep(
      this.test,
      this.homePageSearchArea,
      "fill",
      "search for " + currentItem + "in sear box",
      currentItem
    );
  ...
  async applifiltersToSearch() {
    await actions.executeStep(
      ...
      this.itemSortButton,
      "click",
      "click on short button for sort items"
    );
    await actions.executeStep(
      this.test,
      this.bestSellerItemButton,
      "click",
      "click on best seller in sort items"
    );
    await actions.executeStep(
      this.test,
      this.sortApplyButton,
      "click",
      "click in apply sort button"
    );
  }
  ...
  async addItemToCart() {
    await actions.executeStep(
      this.test,
      this.productAddToCartButton,
      "click",
      "click on product addto cart button"
    );
  }
